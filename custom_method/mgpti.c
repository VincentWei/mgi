...
static int copy_words_from_source (const char* source, char* buff, int copied,
                int case_mode)
{
    int i, len;
    char* space=NULL;
    ...
    while (source) {
        ...
        if (space == NULL) {
            ...
            if (copied + len < PTI_RESULT_BUFF_LONG) {
...
static int write_matched_words (char* buff, char* cursor, int case_mode)
{
    int copied = 0;
    ...
    if (cursor < _matched_buffer + SIZE_CW_SPACE) {
        copied = copy_words_from_source (cursor, buff, 0, case_mode);
    ...
    if (copied < PTI_RESULT_BUFF_LONG) {
        int new_copied;

        if (cursor < _matched_buffer + SIZE_CW_SPACE)
            cursor = _matched_buffer + SIZE_CW_SPACE + 1;

        new_copied = copy_words_from_source (cursor, buff, copied, case_mode);
                ...
                int case_mode) 
{
    ...
    if ((isdigit (*keystokes) 
        ...
        return write_matched_words (buff, (char*)cursor, case_mode);
    }
    else {
    ...
    memset (buff, 0, PTI_RESULT_BUFF_LONG);
